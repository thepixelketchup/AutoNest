...
export function MemberModal({ isOpen, onClose, onSave, defaultValues = null }) {
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <h2 className="text-xl font-bold text-gray-900">{defaultValues ? 'Edit Member' : 'Add New Member'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Contributor Name</label>
              <input 
                type="text" 
                name="name"
                value={formData.name}
          ...
          <div className="pt-4 flex justify-end space-x-3">
            ...
            <Button type="submit" variant="primary" className="px-8 shadow-sm">{defaultValues ? 'Save Changes' : 'Add Member'}</Button>
          </div>
        </form>
      </div>
    </div>
